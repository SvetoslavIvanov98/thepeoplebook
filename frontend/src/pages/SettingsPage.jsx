    ...
    >
      ...
      >
        ...
        <p className="text-sm text-gray-500">
          You have the right to appeal this decision under the EU Digital Services Act. Explain why
          you believe this decision should be reconsidered.
        </p>
        <textarea
          value={note}
  ...
  return (
    <div>
      ...
      <section className="p-4 border-b border-gray-200 dark:border-gray-800">
        <h2 className="font-bold text-base mb-3">Security</h2>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-end pt-1">
              <button
                ...
                disabled={!currentPassword || newPassword.length < 8 || updatingPassword}
                className="px-4 py-2 text-sm font-semibold rounded-lg bg-brand-600 text-white hover:bg-brand-700 disabled:opacity-50"
              >
          ...
          {(decisions || []).map((d) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                    d.action_type === 'warning'
                      ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400'
                      : d.action_type === 'content_removed'
                        ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'
                        : d.action_type === 'account_suspended'
                          ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'
              ...
              <div className="text-sm space-y-1">
                <p>
                  <strong className="text-gray-700 dark:text-gray-300">Reason:</strong> {d.reason}
                </p>
                {d.report_reason && (
                  <p>
                    <strong className="text-gray-700 dark:text-gray-300">Report category:</strong>{' '}
                    {d.report_reason.replace(/_/g, ' ')}
                  </p>
                )}
                {d.legal_basis && (
                  <p>
                    <strong className="text-gray-700 dark:text-gray-300">Legal basis:</strong>{' '}
                    {d.legal_basis}
                  </p>
                )}
              </div>
              ...
              {d.appealed ? (
                <div
                  className={`text-xs rounded-lg p-2 ${
                    d.appeal_outcome === 'overturned'
                      ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400'
                      : d.appeal_outcome === 'upheld'
