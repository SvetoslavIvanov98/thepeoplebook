...
function formatTime(d) { return format(new Date(d), 'h:mm a'); }
...
function buildGroups(messages, myId) {
  ...
  for (const msg of messages) {
    ...
    if (cur && cur.sender_id === msg.sender_id && gap < 300000) {
      ...
      cur = { isMine, sender_id: msg.sender_id, username: msg.username, avatar_url: msg.avatar_url, messages: [msg] };
      groups.push(cur);
    }
  }
  return groups;
...
function MiniChat({ conversation, user, onBack }) {
  ...
  const partnerAvatar = conversation.partner_avatar || ('https://ui-avatars.com/api/?name=' + conversation.partner_username + '&size=48');
  ...
  return (
    <>
      <div className='flex items-center gap-2 px-3 py-2.5 bg-brand-600 rounded-t-2xl flex-shrink-0'>
        <button onClick={onBack} className='text-white/80 hover:text-white flex-shrink-0'><IconBack /></button>
        <img src={partnerAvatar} alt='' className='w-7 h-7 rounded-full object-cover flex-shrink-0' />
        <span className='font-semibold text-white text-sm truncate flex-1'>{conversation.partner_name || conversation.partner_username}</span>
      </div>
      <div className='flex-1 overflow-y-auto py-2 bg-white dark:bg-gray-950'>
        {groups.map((group) => {
          ...
          return (
            <div key={group.messages[0].id} className={'flex flex-col mb-1 px-3 ' + (group.isMine ? 'items-end' : 'items-start')}>
              <div className={'flex items-end gap-1.5 ' + (group.isMine ? 'flex-row-reverse' : 'flex-row')}>
                {!group.isMine && <img src={group.avatar_url || ('https://ui-avatars.com/api/?name=' + group.username + '&size=40')} alt='' className='w-5 h-5 rounded-full object-cover flex-shrink-0 self-end mb-0.5' />}
                <div className={'flex flex-col gap-[2px] max-w-[85%] ' + (group.isMine ? 'items-end' : 'items-start')}>
                  {group.messages.map((m, i) => (
                    <div key={m.id} className='group/b relative'>
                      <div className={'px-2.5 py-1.5 text-xs leading-relaxed break-words ' + bubbleRadius(group.isMine, i === 0, i === count - 1) + ' ' + (group.isMine ? 'bg-brand-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-100')}>
                        {m.content}
                        {m.media_url && <img src={m.media_url} alt='' className='mt-1 rounded-lg max-w-full max-h-40 object-cover' />}
                      </div>
                      <span className={'absolute top-1/2 -translate-y-1/2 pointer-events-none opacity-0 group-hover/b:opacity-100 transition-opacity text-[10px] text-gray-400 whitespace-nowrap ' + (group.isMine ? 'right-full mr-1.5' : 'left-full ml-1.5')}>{formatTime(m.created_at)}</span>
                    </div>
                  ))}
                </div>
      ...
      <div className='flex-shrink-0 flex items-center gap-1.5 px-2 py-2 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-950 rounded-b-2xl'>
        <input value={text} onChange={handleChange} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); if (canSend) send(); } }} placeholder='Aa' className='flex-1 bg-gray-100 dark:bg-gray-800 rounded-full px-3 py-1.5 text-xs outline-none text-gray-900 dark:text-gray-100 placeholder-gray-400' />
        <button onClick={() => canSend && send()} disabled={!canSend} className={'flex-shrink-0 transition-colors ' + (canSend ? 'text-brand-600 hover:text-brand-700' : 'text-gray-300 dark:text-gray-600')}><IconSend /></button>
      </div>
    </>
  );
}
  ...
  return (
    <>
      ...
      <div className='flex-1 overflow-y-auto'>
        ...
        {!isLoading && (conversations || []).length === 0 && <p className='text-center text-gray-400 text-xs py-6'>No conversations yet</p>}
        {(conversations || []).map((c) => (
          <button key={c.id} onClick={() => onSelect(c)} className='w-full flex items-center gap-3 px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-800 border-b border-gray-100 dark:border-gray-800 text-left'>
            <img src={c.partner_avatar || ('https://ui-avatars.com/api/?name=' + c.partner_username)} alt='' className='w-9 h-9 rounded-full object-cover flex-shrink-0' />
            <div className='flex-1 min-w-0'>
              <div className='flex items-center justify-between gap-1'>
                <p className={'text-sm truncate ' + (c.unread_count > 0 ? 'font-bold text-gray-900 dark:text-white' : 'font-medium text-gray-800 dark:text-gray-200')}>{c.partner_name || c.partner_username}</p>
                {c.last_message_at && <span className='text-[10px] text-gray-400 flex-shrink-0'>{formatPreview(c.last_message_at)}</span>}
              </div>
              <p className={'text-xs truncate ' + (c.unread_count > 0 ? 'font-semibold text-gray-700 dark:text-gray-300' : 'text-gray-500')}>{c.last_message || 'No messages yet'}</p>
            </div>
            {c.unread_count > 0 && <span className='w-4 h-4 bg-brand-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center flex-shrink-0'>{c.unread_count}</span>}
          </button>
        ))}
      </div>
    </>
...
export default function FloatingMessageBubble() {
  ...
  useEffect(() => { if (open || pathname.startsWith('/messages')) clearUnreadMessages(); }, [open, pathname]);
  useEffect(() => { if (pathname.startsWith('/messages')) setOpen(false); }, [pathname]);
